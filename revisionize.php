...
function init() {
    ...
    if (is_admin() && user_can_revisionize()) {
        ...
        add_action('post_submitbox_start', __NAMESPACE__ . '\\post_button', 10, 2);
...
function post_button() {
    global $post;
    if (strpos(strtolower($post->post_type), 'acf') !== false) {
        return;
    }
...
function admin_actions($actions, $post) {
    if (is_create_enabled($post)) {
        $actions['create_revision'] = '<a href="' . get_create_link($post) . '" title="' . esc_attr(__("Create a Revision", REVISIONIZE_I18N_DOMAIN)) . '">' . __('Revisionize', REVISIONIZE_I18N_DOMAIN) . '</a>';
    }
